...
current = 0
for day in reversed(ordered_dates):
    if counts[day] > 0:
        current += 1
    else:
        break

_, longest = streaks([counts[d] for d in ordered_dates])
...
stats = header("GITHUB METRICS", 350)
metrics = [
...
stats += f'<text x="42" y="335" fill="#64748b" font-family="monospace" font-size="12">LIVE DATA • @{esc(USER)} • GITHUB ACTIONS</text></svg>'
...
(OUT / "top-languages.svg").write_text(langs, encoding="utf-8")

streak = header("CONTRIBUTION STREAK", 280)
streak += f'<text x="60" y="135" fill="#7dd3fc" font-family="monospace" font-size="13">CURRENT STREAK</text>'
streak += f'<text x="60" y="175" fill="#fff" font-family="monospace" font-size="32" font-weight="700">{current} DAYS</text>'
streak += f'<text x="360" y="135" fill="#7dd3fc" font-family="monospace" font-size="13">LONGEST STREAK</text>'
streak += f'<text x="360" y="175" fill="#fff" font-family="monospace" font-size="32" font-weight="700">{longest} DAYS</text>'
streak += f'<text x="660" y="135" fill="#7dd3fc" font-family="monospace" font-size="13">TOTAL</text>'
streak += f'<text x="660" y="175" fill="#fff" font-family="monospace" font-size="32" font-weight="700">{calendar["totalContributions"]}</text>'
streak += f'<text x="60" y="230" fill="#64748b" font-family="monospace" font-size="12">CALCULATED FROM GITHUB CONTRIBUTION CALENDAR • @{esc(USER)}</text></svg>'
(OUT / "contribution-streak.svg").write_text(streak, encoding="utf-8")
